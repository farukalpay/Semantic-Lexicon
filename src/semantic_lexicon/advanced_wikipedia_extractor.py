...
from .wikipedia_extractor import WikipediaTerm
...
class AdvancedWikipediaExtractor:
    ...
    def _explore_topic(self, page_title: str, depth: int, parent_relevance: float):
        ...
        for i, (link_title, link_score) in enumerate(scored_links[:self.max_pages_per_level]):
    ...
    def _get_linked_pages(self, page_title: str) -> List[str]:
        ...
        try:
            ...
            filtered_links = [
                link.get('title', '')
                for link in links
                if not link.get('title', '').startswith(('Template:', 'Help:', 'Wikipedia:', 'File:', 'Category:'))
    ...
    def _score_linked_pages(self, linked_pages: List[str], parent_categories: List[str]) -> List[Tuple[str, float]]:

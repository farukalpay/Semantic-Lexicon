...
from .wikipedia_extractor import WikipediaTermExtractor, WikipediaTerm, WikipediaFact
...
class DirectQAGenerator:
    ...
    def _build_explanation_from_facts(self, topic: str, fact_groups: Dict) -> str:
        ...
        if fact_groups["capabilities"]:
            ...
            if len(unique_capabilities) == 1:
                ...
                cap_sentence = f"This enables both {unique_capabilities[0]} and {unique_capabilities[1]}."
                paragraphs.append(cap_sentence)
    ...
    def _build_how_answer(self, topic: str, terms: List[WikipediaTerm]) -> str:
        ...
        if definitions:
            sentences.append(f"To understand how {topic} works, note that it is {definitions[0].term}.")
        ...
        return " ".join(sentences) if sentences else f"The mechanism of {topic} requires detailed technical explanation."
    ...
    def _build_why_answer(self, topic: str, terms: List[WikipediaTerm]) -> str:
        ...
        if years:
            sentences.append(f"It was developed in {years[0].term} to address specific computational needs.")
        ...
        return " ".join(sentences) if sentences else f"The rationale for {topic} stems from practical needs."
    ...
    def _build_how_answer_from_facts(self, topic: str, fact_groups: Dict) -> str:
        ...
        if fact_groups["definitions"]:
            fact = fact_groups["definitions"][0]
            intro = f"To understand how {topic} works, it helps to know that it {self._naturalize_phrase(fact.object)}."
            paragraphs.append(intro)
        
        # Explain the mechanism through capabilities
        if fact_groups["capabilities"]:
            capabilities = [self._naturalize_phrase(f.object) for f in fact_groups["capabilities"][:3]]
        ...
        if not fact_groups["capabilities"] and not fact_groups["composition"]:
            paragraphs.append(f"The specific workings of {topic} depend on the implementation context.")
    ...
    def _build_why_answer_from_facts(self, topic: str, fact_groups: Dict) -> str:
        ...
        if fact_groups["definitions"]:
            fact = fact_groups["definitions"][0]
            paragraphs.append(f"{topic.capitalize()} exists because it provides {self._naturalize_phrase(fact.object)}.")
        
        # Explain benefits through capabilities
        if fact_groups["capabilities"]:
            capabilities = [self._naturalize_phrase(f.object) for f in fact_groups["capabilities"][:2]]
            if capabilities:
        ...
        if fact_groups["temporal"]:
            ...
            if fact_groups["attribution"]:
                ...
                paragraphs.append(f"It was developed in {year} by {creator} to address emerging needs.")
            else:
        ...
        if len(paragraphs) == 0:
            paragraphs.append(f"The rationale for {topic} comes from practical requirements in the field.")
    ...
    def _build_temporal_answer(self, topic: str, fact_groups: Dict) -> str:
        ...
        if fact_groups["temporal"]:
            ...
            if fact_groups["attribution"]:
                creator = fact_groups["attribution"][0].object
                paragraphs.append(f"While the exact date is unclear, {topic} is associated with {creator}.")
            else:
                paragraphs.append(f"The specific timeline for {topic} would require more historical context.")
    ...
    def _build_attribution_answer(self, topic: str, fact_groups: Dict) -> str:
        ...
        if fact_groups["attribution"]:
            ...
            if fact_groups["temporal"]:
                year = fact_groups["temporal"][0].object
                paragraphs.append(f"While {topic} emerged around {year}, specific attribution is unclear.")
            else:
                paragraphs.append(f"The specific creators of {topic} would require more historical research.")
    ...
    def _build_answer_from_terms(self, topic: str, terms: List[WikipediaTerm], question_type: str) -> str:
        ...
        if question_type in ["explain", "what"]:
            ...
            # General answer
            if term_groups["definitions"]:
                sentences.append(f"{topic.capitalize()} can be described as {term_groups['definitions'][0].term}.")
            if term_groups["technical"]:
...
class EnhancedSentenceBuilder:
    ...
    def _build_technical_sentence(self, topic: str, facts: Dict) -> str:
        ...
        return f"The technical implementation varies by application"

...
class WikipediaTermExtractor:
    ...
    def _extract_facts_from_text(self, text: str, source_title: str, topic: str) -> List[WikipediaFact]:
        ...
        for sentence in sentences[:10]:
            ...
            if " is a " in sentence.lower() or " is an " in sentence.lower():
                ...
                for pattern in [
                    ...
                    r'([^,]+?)\s+is\s+an?\s+([^,]+?)(?:,\s*(?:which|that|and))',  # Until conjunction
                ]:
                    ...
                    if match:
                        ...
                        if topic_normalized in subject.lower() or subject.lower() in ['it', 'this'] or len(subject.split()) <= 3:
                            # Clean and validate the object
                            clean_obj = self._clean_object(obj)
                            
                            # Make sure we got a meaningful definition
                            if len(clean_obj.split()) >= 3:  # At least 3 words for a meaningful definition
            ...
            if any(word in sentence.lower() for word in ["can", "enables", "allows", "used to", "used for"]):
                patterns = [
                ...
                for pattern in patterns:
                    ...
                    if match:
                        ...
                        if len(capability.split()) >= 2:  # At least 2 words
                            ...
                            if not any(f.object == capability for f in facts if f.predicate == "enables"):
                                facts.append(WikipediaFact(
                                    subject=subject,
                                    predicate="enables",
                                    object=capability,
            ...
            by_match = re.search(r'(?:developed|created|introduced|designed)\s+by\s+([A-Z][^,\.]+)', sentence)
            if by_match:
    ...
    def _extract_terms_from_text(self, text: str, source_title: str, topic: str) -> List[WikipediaTerm]:
        ...
        for i, word in enumerate(words):
            if word in ["explain", "what", "define", "describe"] and i + 1 < len(words):
                ...
                if i + 2 < len(words):
                    ...
                    if not any(w in ["is", "are", "the", "a", "an"] for w in [words[i+1], words[i+2]]):
